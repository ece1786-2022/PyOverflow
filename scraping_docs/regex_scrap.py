...
def get_sub_topics_regex(bes, depth=0, name=None):
    ...
    if topics:
        ...
        for topic in topics:
            ...
            functions = topics_section.find_all("dl", recursive=False)
            # functions.extend(topics_section.find_all("dl", class_="py function", recursive=False))
            func_lst = []
            if functions:
                for func in functions:
                    index = 0
                    all_names = func.find_all("dt")
                    all_dd = func.find_all("dd")
                    for name in all_names:
                        if len(all_dd) > index:
                            func_dict = {}
                            func_name = name.text
                            func_dict['name'] = func_name
                            func_dict['content'] = [p.text for p in all_dd[index].find_all("p", recursive=False)]
                            func_dict['code_demo'] = [p.text for p in all_dd[index].find_all("div", recursive=False)]
            ...
            topic_dict['Subtopics'] = get_sub_topics_regex(topics_section, depth + 1, topic_name)
...
if __name__ == '__main__':
    ...
    # print(topics_section)
    data = get_sub_topics_regex(bes)
    with open("regex.json", "w") as final:
